...
- 插值：位置按 `path_step_m`、姿态按 `attitude_step_deg`（测地转角）分别细分，
  每段取两者较大分段数；欧拉角先逐轴展开（最短分支）再线性插值。复合
  yaw/pitch/roll 下 Euler 插值的相邻实际转角可能远大于端点测地角估计
  （近万向锁区域），分段数按实际相邻转角迭代加密，确保不超步长；
  所需采样量超 `max_samples` 时等比放宽并列 `CLEARANCE_SAMPLING_CAP` 证据缺口，
  不静默给出安全结论。
- 校核：包络中心 = 吊钩位置 + R·偏移，定向包络（OBB）与轴对齐盒（AABB）用
  分离轴法（15 条候选轴）求有符号净空，逐点再减去扫掠修正（相邻采样区间包络
  表面点最大位移的一半），得到覆盖采样区间的保守净空——采样点之间穿过
  薄障碍也会判碰撞。
