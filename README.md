...
载荷表净额定能力（同一臂长与回转区段的相邻档位内插值、不外推）、支腿反力与垫板接地压力，
定位首个超载 / 支腿拔起 / 越出载荷表 / 地基超压区间；结果进入 `approvable` 与版本差异，
批准版冻结载荷表摘要与路径，JSON 与 SVG 站位图共用同一份逐姿态数据。
...
- 路径按 `path_step_m` 在相邻姿态间插值，逐点计算作业半径、净额定载荷
  ...
  吊钩载荷直接复用吊装核算的吊钩动载。
- 载荷表只在同一臂长与回转区段的相邻半径档位间线性插值，越界即 `CRANE_OUT_OF_CHART`，不外推。
